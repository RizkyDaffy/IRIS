...
> **⚠️ Use this exact flow, not a bare `npm install @sugity/iris-node`.** Field testing surfaced four setup traps that produce silent failures (zero routes synced, or a boot-time crash) rather than a clear error. Each step below exists to close one of them.

**Step 1 — install via the setup script, not a direct install:**
```json
"scripts": {
...
npm run setup:iris
```
Do **not** `rm -rf .iris-tmp` afterward. `--install-links` writes a `file:.iris-tmp/SDK/JS` reference into `package.json`; deleting the source folder breaks the link and the *next* `npm install` anyone runs (CI included) silently drops the SDK. Add `.iris-tmp` to `.gitignore` instead.

**Step 2 — pin Express to 4.x, if applicable:**
```bash
npm install express@^4.21.1 @types/express@^4.17.21
```
`registerExpressApp()` currently only understands Express 4's internals (`_router.stack` / `layer.regexp`). Express 5 hid these behind `layer.matchers`, so on Express 5 the discovery walk fails silently and syncs zero routes — no error, no `discovery:warning`, nothing. Until the SDK is updated for Express 5 (tracked as a fix for Part 2's discovery module, §2.4 Flow B), stay on 4.x.

**Step 3 — separate the API port from the gateway port:**
```env
API_PORT=4000
IRIS_PROJECT_TOKEN=your_generated_token_here
IRIS_GATEWAY_URL=http://localhost:3001
```
If your app listens on the same port the gateway uses, the SDK's requests hit your own backend instead of `iris-core` — your backend 404s, the SDK swallows it, and routes never arrive with no obvious cause.

**Step 4 — construct with an explicit object, and don't let a failed sync crash the app:**
```typescript
import express from 'express';
import { Iris } from '@sugity/iris-node';
import { registerExpressApp } from '@sugity/iris-node/express';

// Always pass {} explicitly. `new Iris()` with zero arguments throws
// (it reads .projectToken off an undefined config) before it can fall
// back to IRIS_PROJECT_TOKEN from .env — `new Iris({})` sidesteps that.
const iris = new Iris({});
const app = express();
...
const server = app.listen(process.env.API_PORT ?? 4000, async () => {
    try {
        ...
        // Non-fatal — a gateway outage at boot shouldn't take the app down with it
...
- *Cause:* `setup:iris` (§0.5.5) used `--install-links`, and someone later ran `rm -rf .iris-tmp`, breaking the `file:` link the install depends on.
- *Fix:* Never delete `.iris-tmp` after `setup:iris`. Add it to `.gitignore` instead of removing it.

**6. Route count shows 0 in portal with no errors logged (JS/Express)**
- *Cause:* The project is on Express 5.x. `registerExpressApp()` reads Express-4-only internals and fails silently on Express 5.
- *Fix:* Pin `express`/`@types/express` to `^4.x` per §0.5.5, Step 2, until the SDK supports Express 5.

**7. `TypeError: Cannot read properties of undefined (reading 'projectToken')` on startup (JS)**
- *Cause:* `new Iris()` was called with zero arguments; the constructor doesn't default its config parameter.
- *Fix:* Always call `new Iris({})`, never bare `new Iris()`.

**8. Routes never appear in portal, backend logs a 404 for the sync request (JS)**
- *Cause:* `API_PORT` collides with `IRIS_GATEWAY_URL`'s port, so sync requests hit the app's own backend instead of the gateway.
- *Fix:* Set `API_PORT` to a distinct value from the gateway port per §0.5.5, Step 3.
...
* **Constructor (sync, throws on programmer error only):** signature is `constructor(config: IrisConfig = {})` — the config parameter **must** default to `{}`. A bare `new Iris()` with zero arguments must behave identically to `new Iris({})`, resolving `projectToken` from `IRIS_PROJECT_TOKEN` exactly as it would if a config object had been passed. An earlier build omitted this default, so `new Iris()` threw a raw `TypeError` reading `.projectToken` off `undefined` before the config-validation logic ever ran — that's not the same failure as the intended one below and must not happen. Once the parameter safely defaults, resolve `projectToken` from explicit config or `IRIS_PROJECT_TOKEN`; throw `IrisConfigError` immediately if missing/malformed, naming the exact fix. No network call in the constructor.
...
* Express: walking `app._router.stack` touches an internal, version-fragile structure — wrap the whole walk in `try/catch`, recursively descend nested routers, skip/normalize regex-only paths, dedupe `(path, method)` pairs, and on any unrecognized shape emit `'discovery:warning'` and return an empty set rather than throwing. **This must explicitly cover the Express 4 → 5 internals change:** Express 4 exposes `layer.regexp` on each stack entry; Express 5 replaced it with `layer.matchers` behind a closure. An earlier build only handled the Express 4 shape and, on Express 5, hit an unhandled `undefined` access that the `try/catch` swallowed *before* the `discovery:warning` emit — the walk failed closed with zero routes and zero signal. Detect the installed Express major version (or the actual shape present on the layer object) and either walk it correctly or emit `discovery:warning` naming the specific version mismatch — never silence a real failure into an empty, unexplained result.
...
Unit tests (mocked `fetch`, fake timers) for retry/backoff timing, 4xx-never-retries, queue backpressure. Discovery fixture tests for nested/regex Express routes and Fastify's `onRoute`, asserting `discovery:warning` fires on malformed input rather than throwing — **fixtures must include both Express 4.x (`layer.regexp`) and Express 5.x (`layer.matchers`) route-table shapes**, run against real installs of each major version rather than a single mocked shape, so a future internals change is caught by CI instead of shipping as a silent zero-route regression. A `new Iris()` (zero-argument) smoke test asserting it doesn't throw and resolves config identically to `new Iris({})`. A CI step that actually `require()`s the CJS build and `import`s the ESM build to catch `exports`-map misconfiguration. Matrix-tested on Node 18/20/22; releases via Changesets with npm provenance.
...
- [ ] `new Iris()` called with zero arguments behaves identically to `new Iris({})` — never throws a raw `TypeError` before config validation runs.
- [ ] Express discovery is tested against both Express 4.x and 5.x internals; a version mismatch emits `discovery:warning` naming the mismatch, never fails silently with zero routes and no signal.
