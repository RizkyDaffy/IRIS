...
**2. "Failed to connect to iris-core: iris-core [401] /admin/applications: Invalid or missing X-Iris-Admin-Key" in Portal**
- *Cause:* The `IRIS_ADMIN_KEY` environment variable is set in the gateway (`iris-core` / STEP1) but is missing from the portal's (`iris-portal` / STEP3) `.env.local` file.
- *Fix:* Copy the `IRIS_ADMIN_KEY` value from the gateway's `.env` and add it to `iris-portal/.env.local`, then restart the portal dev server.

**3. Next.js shows a generic 404 page for `/app/[appId]` but the app exists (e.g. `GET /app/xxx 404`)**
- *Cause:* Previously, the Next.js page components for the portal caught *any* error from the backend (like a `401 Unauthorized` or network failure) and mistakenly called `notFound()`, masking the real error as a 404.
- *Fix:* The `iris-core-client.ts` was updated to throw an `ApiError` with the specific HTTP status code, and the page components were updated to only call `notFound()` if `error.status === 404`. Other errors will now correctly surface as 500-level errors or exact error traces.

**4. Route count shows 0 in portal (PHP/Laravel)**
...
**5. "Failed to connect to iris-core [401]" in Portal**
...
**6. "Unauthorized: invalid token" during `iris:sync` or `iris.init()`**
...
**7. Prisma "Can't reach database server"**
...
> Quick preview of infrastructure updates. Full engineering spec in `agents.md`. Full install guide in `new-docs.md`.

---

### [2026-07-20] — Schema, Auth & Admin API Overhaul

**iris-core (STEP1)**
- `schema.prisma` now explicit: `provider = "mysql"` — not PostgreSQL. XAMPP only.
- `User` model added to gateway schema. Powers portal's built-in auth (no separate auth service).
- `Role` enum: `SUPER_ADMIN | ADMIN | DEVELOPER`. `UserStatus` enum: `PENDING | ACTIVE | REJECTED`.
- New env vars: `METRICS_KEY` (gates `/metrics`), `EVENT_RETENTION_DAYS` (outbox purge, default 90).
- `securityAudit.ts` route file added (`/security/*` endpoints).
- Token rotation (`POST /admin/applications/:id/rotate-token`) now immediately evicts in-process cache.
- Events endpoint (`GET /admin/applications/:id/events`) now cursor-paginated with `status` filter.

**iris-portal (STEP3)**
- Auth is **custom JWT** (not SSO/OIDC) — `jose` HS256, 24h httpOnly cookie.
- New required env var: `JWT_SECRET` in `.env.local`.
- New routes: `/login`, `/daftar` (register → PENDING), `/queue` (pending holding page), `/admin` (approve/reject users).
- Middleware redirects PENDING users → `/queue`, ACTIVE users away from `/queue`.
- Portal reads all data via iris-core `/admin/*` API — no direct DB access.
- `ApiError` now carries HTTP status — 401/500 no longer masked as 404 in Next.js pages.

**iris-node SDK (STEP2)**
- `iris.flush(timeoutMs?)` added — best-effort queue drain on shutdown.
- `iris.registerRoutes` alias added (same as `syncRoutes`) for discovery packages.
...
| Feature | Status |
|---------|--------|
| Zero-config auto-registration handshake | ❌ TODO — token still created manually via admin API |
| Real-time SSE event stream in portal | ❌ TODO |
| Token reveal dialog in portal UI | ❌ TODO |
| `@iris/db` shared package | ❌ Not needed yet — portal uses HTTP API |
